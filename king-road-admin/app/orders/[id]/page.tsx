...
export default function OrderDetailsPage() {
  ...
  const fetchOrder = async () => {
    try {
      setLoading(true);
      const orderData = await useGetOrderById(orderId);
      setOrder(orderData);
    } catch (error) {
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setTrackingData(prev => ({ ...prev, shipping_method: e.target.value }))}
                ...
                <Button onClick={handleTrackingUpdate} disabled={updating || !trackingData.tracking_number}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {order.tracking_number && (
                  <>
                    ...
                    <div>
                      ...
                      <p className="mt-1">{order.shipping_method || 'N/A'}</p>
                    </div>
                  </>
                )}
                {order.estimated_delivery && (
                  <div className="col-span-2">
                    ...
                    <p className="mt-1">{formatDate(order.estimated_delivery)}</p>
                  </div>
                )}
              </div>
            </CardContent>
              ...
              <CardContent className="space-y-4">
                {order.customer_notes && (
                  <div>
                    ...
                    <p className="mt-1 p-3 bg-blue-50 rounded-md">{order.customer_notes}</p>
                  </div>
                )}
                {order.internal_notes && (
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Internal Notes</Label>
                    <p className="mt-1 p-3 bg-gray-50 rounded-md whitespace-pre-wrap">{order.internal_notes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-1">
                  <Badge variant={order.is_guest_order ? "secondary" : "default"}>
            ...
            <CardContent>
              <div className="space-y-2">
                <Badge variant="outline" className="mb-2">
                  {order.address_details.type.charAt(0).toUpperCase() + order.address_details.type.slice(1)}
                </Badge>
                <p className="text-sm leading-relaxed">{order.formatted_address}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <p className="mt-1 capitalize">{order.payment_method || 'N/A'}</p>
              </div>
              <div>
                <Label className="text-sm font-medium text-gray-500">Payment Status</Label>
                <div className="mt-1">
              ...
              {order.payment_reference && (
                <div>
                  ...
                  <p className="mt-1 font-mono text-sm">{order.payment_reference}</p>
                </div>
              )}
            </CardContent>
          </Card>
